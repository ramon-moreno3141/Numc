...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    ...
    Matrix61c* addend = (Matrix61c*)args; //cast necessary?
    matrix* result_mat;
    if (allocate_matrix(&result_mat, self->mat->rows, self->mat->cols)) {
        PyErr_SetString(PyExc_RuntimeError, "Memory allocation failed.");
        return NULL;
    }
    if (add_matrix(result_mat, self->mat, addend->mat)) {
        PyErr_SetString(PyExc_TypeError, "Mismatched dimensions.");
        return NULL;
    }
    Matrix61c* Py_Result = (Matrix61c*)Matrix61c_new(&Matrix61cType, NULL, NULL);
    Py_Result->mat = result_mat;
    Py_Result->shape = PyTuple_Pack(2, PyLong_FromLong(result_mat->rows), PyLong_FromLong(result_mat->cols));
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
    ...
    Matrix61c* subend = (Matrix61c*)args; //cast necessary?
    matrix* result_mat;
    if (allocate_matrix(&result_mat, self->mat->rows, self->mat->cols)) {
        PyErr_SetString(PyExc_RuntimeError, "Memory allocation failed.");
        return NULL;
    }
    if (sub_matrix(result_mat, self->mat, subend->mat)) {
        PyErr_SetString(PyExc_TypeError, "Mismatched dimensions.");
        return NULL;
    }
    Matrix61c* Py_Result = (Matrix61c*)Matrix61c_new(&Matrix61cType, NULL, NULL);
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    Matrix61c* mul_operand = (Matrix61c*)args; //cast necessary?
    matrix* result_mat;
    if (allocate_matrix(&result_mat, self->mat->rows, mul_operand->mat->cols)) {
        PyErr_SetString(PyExc_RuntimeError, "Memory allocation failed.");
        return NULL;
    }
    if (mul_matrix(result_mat, self->mat, mul_operand->mat)) {
        PyErr_SetString(PyExc_TypeError, "Mismatched dimensions.");
        return NULL;
    }
    Matrix61c* Py_Result = (Matrix61c*)Matrix61c_new(&Matrix61cType, NULL, NULL);
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    ...
    if (neg_matrix(result_mat, self->mat)) {
...
static PyObject *Matrix61c_abs(Matrix61c *self) {
    ...
    if (abs_matrix(result_mat, self->mat)) {
        PyErr_SetString(PyExc_TypeError, "Invalid input.");
        return NULL;
    }
    Matrix61c* Py_Result = (Matrix61c*)Matrix61c_new(&Matrix61cType, NULL, NULL);
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    ...
    int power;
    if (!PyLong_Check(pow) || ((power = PyLong_AsLong(pow)) <= 0)) {
        PyErr_SetString(PyExc_TypeError, "Power is not valid.");
        return NULL;
    }
    matrix* result_mat;
    if (allocate_matrix(&result_mat, self->mat->rows, self->mat->cols)) {
    ...
    if (pow_matrix(result_mat, self->mat, power)) {
        PyErr_SetString(PyExc_RuntimeError, "Memory allocation failed.");
        return NULL;
    }
    Matrix61c* Py_Result = (Matrix61c*)Matrix61c_new(&Matrix61cType, NULL, NULL);
...
static PyMethodDef Matrix61c_methods[] = {
    {"get", (PyCFunction)Matrix61c_get_value, METH_VARARGS, "Get the value at the specified row and column of the self object."},
    {"set", (PyCFunction)Matrix61c_set_value, METH_VARARGS, "Set the value at the specified row and column of the self object."},
    {NULL, NULL, 0, NULL}
};
